...
import { KwitansiTemplate, renderKwitansiPdf, getKwitansiFilename } from "@/components/kwitansi";
import type { KwitansiData } from "@/components/kwitansi";

/**
 * Halaman kwitansi publik (shortcut): tidak perlu login.
 * Buka via scan QR di kwitansi cetak → data keluar → tinggal print.
 * URL: /kwitansi?token=xxx
 */
export default function KwitansiShortcutPage() {
  ...
  useEffect(() => {
    if (!token) {
      queueMicrotask(() => {
        setError("Token tidak ada. Buka link cetak kwitansi dari UKT.");
        setLoading(false);
      });
      return;
    }
    fetch(`/api/kwitansi/verify?token=${encodeURIComponent(token)}`, {
      ...
      .then(setData)
      .catch(() => setError("Kwitansi tidak ditemukan atau token tidak valid."))
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      const doc = new jsPDF();
      renderKwitansiPdf(doc, data, qrDataUrl);
      doc.save(getKwitansiFilename(data));
    } catch {
