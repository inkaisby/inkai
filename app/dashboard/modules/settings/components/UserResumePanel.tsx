...
export default function UserResumePanel({ user, refreshTrigger = 0 }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [structRes, funcRes, logRes] = await Promise.all([
        ...
        supabase.from("user_functional_roles").select("id, role_name, active").eq("user_id", user.user_id),
        fetch(`/api/activity/logs?email=${encodeURIComponent(user.email)}`, { credentials: "include" }),
      ]);
  ...
  const levelLabel = user.structural_level != null ? LEVEL_LABELS[user.structural_level] ?? `Level ${user.structural_level}` : null;

  return (
    <div className="space-y-6">
      {/* ================= IDENTITAS ================= */}
      <section className="rounded-xl border border-white/10 bg-white/5 p-5">
        <h3 className="text-sm font-semibold text-cyan-300/90 mb-3">Identitas</h3>
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm">
          ...
          <div>
            ...
            <dd>{user.structural_role ? formatRoleLabel(user.structural_role) : "—"}</dd>
          </div>
          <div>
          ...
          <div>
            <dt className="text-white/50">Domisili (regency_id)</dt>
            <dd>{user.regency_id != null ? String(user.regency_id) : "—"}</dd>
          </div>
          <div>
      ...
      {/* ================= JABATAN STRUKTURAL ================= */}
      <section className="rounded-xl border border-white/10 bg-white/5 p-5">
        <h3 className="text-sm font-semibold text-cyan-300/90 mb-3">Jabatan organisasi (struktural)</h3>
        {structural.length === 0 ? (
          <p className="text-sm text-white/50">Belum ada jabatan. Atur di tab Role Management.</p>
        ) : (
          <ul className="space-y-2 text-sm">
            {structural.map((r) => {
              const org = r.ranting_nama ?? r.cabang_nama ?? r.provinsi_nama;
              const level = LEVEL_LABELS[r.structural_level] ?? `Level ${r.structural_level}`;
              return (
                <li key={r.id} className="flex flex-wrap items-baseline gap-2">
                  ...
                  {!r.active && <span className="text-amber-400/90 text-xs">(nonaktif)</span>}
                </li>
              );
            })}
          </ul>
      ...
      {/* ================= ROLE FUNGSIONAL ================= */}
      <section className="rounded-xl border border-white/10 bg-white/5 p-5">
        <h3 className="text-sm font-semibold text-cyan-300/90 mb-3">Role fungsional</h3>
        {functional.length === 0 ? (
      ...
      {/* ================= AKTIVITAS TERAKHIR ================= */}
      <section className="rounded-xl border border-white/10 bg-white/5 p-5">
        <h3 className="text-sm font-semibold text-cyan-300/90 mb-3">Aktivitas terakhir (20 terbaru)</h3>
        {activity.length === 0 ? (
              ...
              <tbody>
                {activity.map((a) => (
                  <tr key={a.id} className="border-b border-white/5">
                    ...
                    <td className="py-2 text-xs text-white/60 max-w-[200px] truncate" title={a.detail ? JSON.stringify(a.detail) : undefined}>
